...
class ProductTopAttributesCron:
    def __init__(self):
        ...
        self.partition2_offset = 1 #min value is 1, for most recent partition
...
##--------------------------------------------------------------- 
    ...
    def writeNewAttributes(self, write_new_attr, is_new_data, is_prod_db, is_scaled, data_ready):
        if (write_new_attr == 'True') and (is_new_data == 'True'):  ###cron1 job
            try:
                ...
                else:  ##test case for March 22 and 23
                    self.insertTempTableProdAttr(self.db, '20180323', self.temp2_product_attributes, 
                                                 'lhk_qarth_product_22_23', 'today', is_scaled)
                    self.insertTempTableProdAttr(self.db, '20180322', self.temp2_product_attributes, 
                                                 'lhk_qarth_product_22_23', 'yesterday', is_scaled)
    ...
    def writeNewCounts(self, write_new_count, data_ready, is_new_data, is_prod_db):           
        if ((write_new_count == 'True') and (data_ready == 1) and (is_new_data == 'True') or 
                                                         (self.is_data_validation=='True')):   ###cron1 job
            try:
                ####-------------------------------------------------------------------------
                if self.is_data_validation == 'True':  ##test case
                    ...
                    #self.insertTempTableCounts3i(self.db)
    ...
    def getRankedAttributeRawTableau(self, table_name_counts3):
        query = ('select ' + self.dateid2 + ', fqkey3, count(*) as ct '
                'from ' + table_name_counts3 + ' '
                'where trim(fqkey3) != "" '
                'group by fqkey3 '
                'order by ct desc;')
    ...
    def getTop3SourceCountsTableau(self, table_name_counts3, attrs_list_ranked):
        results = []
        attrs_list_top10 = attrs_list_ranked[0:10]
        for attr_name in attrs_list_top10:
    ...
    def generatePostGresQueryTableau(self, attrs_sources_dict):
        ...
        for key_ in attrs_sources_dict:
            ...
            query = 'insert into temp2_qarth_analytics_drilldown (' + attr_names + ') values (' + attr_values + ');'
            print '[queryx]: ', query
            queries.append(query) 
        return queries   
...
def run():
    ...
    #============================For Testing Only===============================================
    #attrs_count_raw = pADD.getRankedAttributeRawTableau('catint.temp2_product_attributes_counts3_bk')
    #print '[attrs_count_raw]: ', attrs_count_raw
    #pADD.writeFile('attrs_count_raw.txt', attrs_count_raw)
    attrs_count_raw = pADD.readFile('attrs_count_raw.txt')
    attrs_dict, attrs_list_ranked = pADD.getRankedAttributesListTableau(attrs_count_raw)
    print '[attrs_list_ranked]: ', attrs_list_ranked[0:10]
    #sources_count_raw_list = pADD.getTop3SourceCountsTableau('catint.temp2_product_attributes_counts3_bk', 
                                                        #attrs_list_ranked)
    #pADD.writeFile('sources_count_raw_list.txt', '\n'.join(sources_count_raw_list))
    sources_count_raw_list = pADD.readFile('sources_count_raw_list.txt')
    print '[sources_count_raw_list]: ', sources_count_raw_list
    attrs_sources_dict = pADD.getAttrsSourcesDictTableau(attrs_dict, sources_count_raw_list)
    print '[attrs_sources_dict]: ', attrs_sources_dict
    queries = pADD.generatePostGresQueryTableau(attrs_sources_dict) 
    queries_s = '\n'.join(queries)
    print '[queries_s]: ', queries_s
    pADD.writeFile('drilldown_queries.txt', queries_s)
    sys.exit(1)
    #===========================================================================================
    if write_new_attr == 'True':
        pADD.insertOrgIds(is_new_ids, is_new_data, is_prod_db) ###This needs to be before inserts into temp2_product_attributes2
        print '[writing new attributes]...'
        data_ready = pADD.writeNewAttributes(write_new_attr, is_new_data, is_prod_db, is_scaled, data_ready)
    #--------------------------------------------------------------------------
    pADD.writeNewCounts(write_new_count, data_ready, is_new_data, is_prod_db)            
    ##  total execution time upto this point:  ~ 10 hrs
    ...
    ## execution time for post_hive section: ~ 4 hrs    
    ...
    #-------------------------------------------------------------- 
    if is_postgres_test == 'True':  ##this block is used for testing only
        pADD.read_top_stats2(pADD.postgres_number_of_days) ## for testing only
        pADD.generatePostgresQuery(pADD.postgres_query_test_file, is_postgres_test)  ## for testing only     
    #-------------------------------------------------------------- 
    end_time = time.time()
